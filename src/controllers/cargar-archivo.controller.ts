  ...
  @post('/CargarImagenCliente', {
    responses: {
      200: {
        ...
        description: 'Función de carga de la imagen de Cliente.',
      },
    },
  })
  ...
  @post('/CargarImagenProyecto', {
    responses: {
      200: {
        ...
        description: 'Función de carga de proyectos.',
      },
    },
  })
  async ImagenCliente(
    ...
    let res = await this.StoreFileToPath(rutaImagenProyecto, llaves.nombreImagenProyecto, request, response, llaves.extensionesPermitidasDOC);
    if (res) {
      const nombre_archivo = response.req?.file.filename;
      if (nombre_archivo) {
        return {filename: nombre_archivo};
